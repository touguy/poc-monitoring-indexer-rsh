...
## 1. 프로젝트 개요
NestJS와 PostgreSQL을 사용하여 이더리움(Sepolia/Mainnet 호환) 네트워크의 Chain Reorg(체인 재조직)를 실시간으로 감지하고, 상태를 추적하며, Reorg 발생 시 DB 기록 및 알림(로그)을 수행하고 지정된 스마트 컨트랙트의 이벤트를 수집하여 DB에 저장하는 백엔드 애플리케이션입니다. 

> [!NOTE]
> 🚀 **[Ponder 확장 아키텍처 도입 계획]**
> 본 프로젝트는 기존의 안정성 위주의 폴링(Polling) 방식에서 고도화된 Ponder Core 아키텍처를 선택적으로 결합할 수 있도록 설계가 확장되었습니다.
> - **옵션 1**: Bloom Filter 기반 RPC 최적화
...
- **`src/contract/`**: 환경 변수(`.env`)로 지정된 스마트 컨트랙트의 이벤트를 `src/block/`의 블록 동기화 시점에 맞춰 RPC `getLogs` 필터로 묶어서 일괄 수집하고, 제너릭 컬럼 DB에 데이터가 분해/저장되도록 하는 이벤트 모니터링 도메인입니다.
