...
### 2. `BlockService` 수정
- `blockQueue`의 타입을 `number[]`에서 `{ number: number, header?: any }[]`로 변경하여 헤더 정보를 함께 담을 수 있게 합니다.
- `onModuleInit`에서 `wsService.onEvent('block')` 대신 `wsService.onBlockHeader()`를 사용하도록 변경합니다.
- `handleNewBlock(blockNumber: number, header?: any)`로 시그니처를 변경합니다.
- `header`가 존재할 경우 RPC 호출 없이 즉시 `BlockRecord`를 생성하고 Reorg 검증을 수행합니다. 만약 헤더 정보가 불충분할 경우에만 기존처럼 RPC 폴백을 수행합니다.
