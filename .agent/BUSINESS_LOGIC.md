...
> [!NOTE]
> 🚀 **[Ponder 확장 기능 - 옵션 5: 대량 동기화 최적화 (Bulk Chunking)]**
> - **변경 계획**: 블록 1개씩 순차적으로 조회하지 않고, 범위를 크게 잡아 수십~수백 개의 블록 단위로 `Promise.all` 청킹을 적용하여 병렬로 수집 속도를 극대화합니다. 노드 Rate Limit 초과 시 재귀적으로 범위를 쪼개는 Dynamic Chunking 기법을 적용합니다.
...
> [!NOTE]
> 🚀 **[Ponder 확장 기능 - 옵션 6: RPC 로컬 캐싱 (RPC Cache Layer)]**
> - **변경 계획**: 한 번 받아온 특정 블록의 데이터(헤더 및 로그)는 메모리에 LRU 기반으로 보관하여, 폴링 및 Reorg 과정에서 동일 블록을 재조회할 때 노드에 트래픽을 유발하지 않게 캐시에서 반환하도록 최적화합니다.
